...
Vagrant.configure("2") do |config|
  ...
  config.vm.provider "virtualbox" do |vb|
      vb.memory = "4096"
      vb.cpus = "2"# https://github.com/hashicorp/vagrant/issues/11777#issuecomment-661076612
      vb.customize ["modifyvm", :id, "--uart1", "0x3F8", "4"]
      vb.customize ["modifyvm", :id, "--uartmode1", "file", File::NULL]

      # use host resolver to increase dl speeds
  ...
  config.vm.provision "shell", inline: <<-SHELL
      ...
      # no need to statrt minikube just yet
  ...
  # alternatively, if running on *nix system, one can use rsync synced folder, as stated below.
